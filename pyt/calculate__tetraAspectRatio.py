...
# ========================================================= #
# ===  calculate aspect ratio of a tetrahedral element  === #
# ========================================================= #

def calculate__tetraAspectRatio( elems=None, nodes=None ):

    # ------------------------------------------------- #
    # --- [1] Arguments                             --- #
    # ------------------------------------------------- #
    if ( elems is None ): sys.exit( "[calculate__tetraAspectRatio.py] elems == ???" )
    if ( nodes is None ): sys.exit( "[calculate__tetraAspectRatio.py] nodes == ???" )

    # - elems :: [ node1, node2, node3, node4 ] :: [ nElems, 4 ]
    # - nodes :: [ x, y, z ]                    :: [ nNodes, 3 ]
    if ( elems.shape[1] != 4 ):
        print( "[calculate__tetraAspectRatio.py] illegal elems shape :: {0} ".format( elems.shape ) )
    if ( nodes.shape[1] != 3 ):
        print( "[calculate__tetraAspectRatio.py] illegal nodes shape :: {0} ".format( nodes.shape ) )
...
if ( __name__=="__main__" ):
    ...
    aspect       = calculate__tetraAspectRatio( elems=elems, nodes=nodes )
